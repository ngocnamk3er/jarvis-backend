...
from app.agents.tools.viz_validate import validate_mermaid


@tool
async def generate_visualization_mermaid(code: str, label: str, title: str = "") -> str:
    ...
    """
    error = await validate_mermaid(code)
    if error:
        return f"Error: invalid Mermaid syntax — {error}"
